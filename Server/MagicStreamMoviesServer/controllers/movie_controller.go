...
import (
	...
	"github.com/MuzakkirJamal/MagicStreamMovies/Server/MagicStreamMoviesServer/database"
	"github.com/MuzakkirJamal/MagicStreamMovies/Server/MagicStreamMoviesServer/models"
	"github.com/MuzakkirJamal/MagicStreamMovies/Server/MagicStreamMoviesServer/utils"
...
func GetReviewRanking(admin_review string, client *mongo.Client, c *gin.Context) (string, int, error) {
	// Step 1: Load rankings from MongoDB
	...
	if OpenAiApiKey == "" {
		log.Println("Warning: OPENAI_API_KEY not set, using fallback sentiment")
		// fallback sentiment
		return "Neutral", 3, nil
	}

	base_prompt_template := os.Getenv("BASE_PROMPT_TEMPLATE")
	if base_prompt_template == "" {
		base_prompt_template = "The sentiment of the following review is: {rankings}"
	}
	...
	if err != nil {
		log.Println("Error initializing OpenAI:", err)
		return "Neutral", 3, nil
	}

	// Step 4: Call OpenAI with error handling
	response, err := llm.Call(c, base_prompt+admin_review)
	if err != nil {
		if strings.Contains(err.Error(), "429") {
			log.Println("OpenAI quota exceeded. Using fallback sentiment.")
			return "Neutral", 3, nil
		}
	...
	// Step 5: Match OpenAI response to ranking value
	rankVal := 0
	for _, ranking := range rankings {
	...
	// Fallback if response doesn't match any ranking
	if rankVal == 0 {
		rankVal = 3
		response = "Neutral"
	}
